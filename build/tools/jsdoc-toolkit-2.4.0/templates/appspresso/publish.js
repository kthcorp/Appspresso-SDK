...
function publish(symbolSet) {
	...
	// create the file index individually - for APPSPRESSO

	for (var fff = 0; fff < files.length; fff++) {
		try {
			var fileindexTemplate = new JSDOC.JsPlate(publish.conf.templatesDir+"overview.tmpl");
		}
		catch(e) { print(e.message); quit(); }
		...
		allFiles.push(new JSDOC.Symbol(files[fff], [], "FILE", new JSDOC.DocComment("/** */")));
		
		print("script file: "+files[fff]);
		allFiles[0] = documentedFiles[fff];
			
		allFiles = allFiles.sort(makeSortby("name"));

		// output the file index page
		var filesIndex = fileindexTemplate.process(allFiles);
		IO.saveFile(publish.conf.outDir, (files[fff].substring(files[fff].lastIndexOf('/'), files[fff].lastIndexOf('.')))+publish.conf.ext, filesIndex);
	}
	fileindexTemplate = filesIndex = files = null;
